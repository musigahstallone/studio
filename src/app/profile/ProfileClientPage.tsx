...
export function ProfileClientPage() { // Renamed from ProfilePage
  ...
  useEffect(() => {
    if (appUser || user) {
      ...
      if (appUser?.transactionTag && (appUser?.name || appUser?.email)) {
        ...
        const recipientName = encodeURIComponent(appUser.name || appUser.email!.split('@')[0]);
    ...
    if (text && typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText(text)
        ...
        .catch(err => {
         ...
         toast({ variant: "destructive", title: "Copy Failed", description: "Clipboard API not available or no text." });
    }
  };
                      ...
                      <div>
                       ...
                       {sendMoneyLink ? (
                          <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                  <Input type="text" value={sendMoneyLink} readOnly className="text-xs h-9 flex-grow bg-muted/50"/>
                                  <Button 
                                      variant="outline" 
                                      size="icon" 
                                      className="h-9 w-9" 
                              ...
                              <p className="text-xs text-muted-foreground mt-1">
                                  Share this link to allow others to send money directly to you via SM Cash.
                              </p>
                          </div>
                      ) : (
