...
*   **Withdrawals:** Withdraw funds from your savings goals when needed. The system supports:
    ...
    *   **Early Withdrawal Logic:** Configure goals to allow or disallow early withdrawals. If allowed, set a percentage-based penalty rate (min 10%) that applies to the *target amount* if funds are withdrawn before maturity.
...
*   **Admin Dashboard (`/admin`):**
    ...
    *   *Note on Advanced Analytics:* The dashboard includes a note that more complex analytics (e.g., per-user aggregates, real-time streaming) would ideally require backend Cloud Functions for scalability, which are not part of the current client-side focused implementation for these stats.
...
*   **Landing Page (`/`):**
    *   Provides an overview of PennyPincher AI, its key features, and how it works.
...
This comprehensive README should provide a solid overview of PennyPincher AI.
